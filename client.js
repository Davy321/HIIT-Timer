...
function advanceTimer() {
  if (level == 0) {
    ...
    if (TOption == 0) {
      ...
      document.getElementById("advancedSettings").innerHTML =
        "Switch to Advanced";
...
function initiateWorkoutTimer() {
  if (!advOption) {
    ...
    for (let i = 0; i < advancedIntervals; i++) {
      ...
      temp2 = Number.isInteger(
        parseInt(document.querySelectorAll(".advTime")[i].value, 10)
      );
      if (temp > 3600 || !temp2) {
      ...
      let object = {
        name: capitalizeFirstLetter(
          document.querySelectorAll(".advName")[i].value
        ),
...
function startAdvancedTimer() {
  if (currentSet >= advancedIntervals) {
    ...
    document.body.style.backgroundColor =
      advancedArray[currentSet].type == "warmup"
        ? "#a9dea0"
        : advancedArray[currentSet].type == "high"
        ? "#ffabab"
        : "#f5f2b5";
    document.getElementById("timerTitle").innerHTML =
      advancedArray[currentSet].name;
...
function createNewInterval() {
  if (advancedIntervals > 0) {
    let x =
      document.getElementById("advancedInputParameters").clientHeight + 292;
    document.getElementById("advancedInputParameters").style.height = x + "px";
  }
  document.getElementById("createInterval").style.borderTop =
    "5px solid #7d7d7d";

  let elem = document.createElement("DIV");
  elem.innerHTML =
    "<label>Name: </label>" +
    '<input class="advName" type="text" value="Interval ' +
    (advancedIntervals + 1) +
    '">' +
    ...
    '<br><button onclick="deleteInterval(' +
    advancedIntervals +
    ')">Delete Interval</button>';
  document
    .getElementById("advancedInputParameters")
    .insertBefore(elem, document.getElementById("createInterval"));
...
function displayImportedTimer(file) {
  ...
  for (let i = 0; i < advancedArray.length; i++) {
    if (advancedIntervals > 0) {
      let x =
        document.getElementById("advancedInputParameters").clientHeight + 292;
      document.getElementById("advancedInputParameters").style.height =
        x + "px";
    }
    document.getElementById("createInterval").style.borderTop =
      "5px solid #7d7d7d";
    ...
    elem.innerHTML =
      "<label>Name: </label>" +
      '<input class="advName" type="text" value="' +
      advancedArray[i].name +
      '">' +
      "<br><label>Duration in seconds: </label>" +
      '<input class="advTime" type="text" value="' +
      advancedArray[i].time +
      '">' +
      "<br><label>Interval Type: </label>" +
      '<select class="advType"><option value="warmup">Warmup/Cooldown</option>' +
      '<option value="high">High Intensity</option>' +
